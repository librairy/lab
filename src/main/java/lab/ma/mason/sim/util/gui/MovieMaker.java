...
/**
   A class which gives a GUI front-end to MovieEncoder.  You create a MovieMaker by passing in some Frame as a parent.  Then you call start(image), where the image is a "typical" sized image for the movie frame.  MovieEncoder will then show dialog panels allowing the user to specify where to save the movie and what format and frame rate to use.  If the user cancels, then start(image) returns false.  Else it returns true and the MovieMaker is ready for action.
...
public class MovieMaker
    ...
    public MovieMaker(Frame parent)
        ...
        try
            {
            encoderClass = Class.forName("MovieEncoder", true, Thread.currentThread().getContextClassLoader());
            }
    ...
    public synchronized boolean start(BufferedImage typicalImage, float fps)
        ...
        try
            ...
            if (fd.getFile()!=null) 
                {
                //                encoder = new MovieEncoder(fps,  // frames per second
    ...
    public synchronized boolean add(BufferedImage image)
        {
        if (!isRunning) return false;
        //              ((MovieEncoder)encoder).add(image);
                {

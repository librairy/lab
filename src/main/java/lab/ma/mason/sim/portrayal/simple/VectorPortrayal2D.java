...
import lab.ma.mason.sim.util.*;
import lab.ma.mason.sim.util.gui.ColorMap;
    ...
    public Inspector getInspector(LocationWrapper wrapper, GUIState state)
        ...
        if (portrayal instanceof ValueGridPortrayal2D)
	        ...
	        if (((ValueGridPortrayal2D) portrayal).getField() instanceof DoubleGrid2D)
	            return Inspector.getInspector(new ValuePortrayal2D.DoubleFilter(wrapper), state, "Properties");
	        else // IntGrid2D
	            return Inspector.getInspector(new ValuePortrayal2D.IntFilter(wrapper) ,state, "Properties");
            }
